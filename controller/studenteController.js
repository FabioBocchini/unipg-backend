...
const checkEmail = (request, response) => {
  ...
  let value = false
  pool.query('SELECT * FROM studente WHERE email = $1 OR matricola = $2',[email, matricola],
  (error, results) =>  {
    if (error) {
      throw error
    }
...
const postStudLogin = (request, response) => { 
  ...
  pool.query('SELECT matricola, nome, cognome, password FROM studente WHERE email = $1', [email], 
  (error, results) => {
    if (error) {
      throw error
    }
...
const postNuovoStudente = (request, response) => { 
  ...
  pool.query('INSERT INTO studente VALUES($1,$2,$3,$4,$5)', [matricola,nome,cognome,email,password], 
  (error, results) => {
    if (error) {
      throw error
    }

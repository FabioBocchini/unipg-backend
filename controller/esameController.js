...
const rifiutaEsame = (request, response) => {
  ...
  pool.query('DELETE FROM esame WHERE matricola = $1 AND id_corso = $2', 
  [matricola,corso], (error, results) => {
    if (error) {
      throw error
    }
...
const getStudEsami = (request, response) => { 
  ...
  pool.query('SELECT corso.id_corso, corso.nome as esame, professore.nome,professore.cognome, esame.dataesame, esame.voto, esame.statoesame FROM esame JOIN corso ON esame.id_corso = corso.id_corso JOIN professore on corso.professore = professore.id AND esame.matricola = $1',
   [matricola], 
  (error, results) => {
    if (error) {
      throw error
...
const inserisciVoto = (request, response) =>{
  ...
  pool.query("INSERT INTO esame VALUES ( $1 , $2 , $3 , $4 ,'Waiting')",[corso,matricola,voto,giorno],
  (error, results) => {
    if (error){
      throw error
    }
